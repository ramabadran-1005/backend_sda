// app.cjs — FINAL STABLE VERSION (Render + MongoDB Atlas)
...
const MONGO_URI = process.env.MONGO_URI; // MUST come from Render env
...
app.get("/api/live-nodes", async (req, res) => {
  ...
  for (const r of rows) {
    ...
    const ts = r.receivedAt ? new Date(r.receivedAt).getTime() : null;
    if (!ts) continue;
